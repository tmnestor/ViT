...
from transformer_swin import create_swin_transformer, save_model
...
def train_model(
    ...
    # Initialize model using Hugging Face implementation
    model = create_swin_transformer(pretrained=True).to(device)
    print("Initialized Swin Transformer model using Hugging Face transformers")
    ...
    for epoch in range(epochs):
        ...
        if val_balanced_acc > best_balanced_acc:
            ...
            save_model(model, os.path.join(output_dir, "receipt_counter_swin_best_bacc.pth"))
        ...
        if val_f1_macro > best_f1_macro:
            ...
            save_model(model, os.path.join(output_dir, "receipt_counter_swin_best_f1.pth"))
            print(f"Saved model with best F1 macro: {val_f1_macro:.2%}")
            improved = True
        
        # Get a comprehensive best model (saves over the above models)
        if improved:
            save_model(model, os.path.join(output_dir, "receipt_counter_swin_best.pth"))
            patience_counter = 0
        else:
            patience_counter += 1
    ...
    # Save final model
    save_model(model, os.path.join(output_dir, "receipt_counter_swin_final.pth"))
    ...
    accuracy, balanced_accuracy = plot_results(
        predictions,
        ground_truth,
        output_path=os.path.join(output_dir, "swin_classification_results.png"),
    )

    # Save training history
    pd.DataFrame(history).to_csv(
        os.path.join(output_dir, "swin_classification_history.csv"), index=False
    )
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "swin_classification_curves.png"))
...
def main():
    ...
    parser.add_argument(
        "--train_csv", required=True, help="Path to CSV file containing training data"
    )
    parser.add_argument(
        "--train_dir", required=True, help="Directory containing training images"
    )
    parser.add_argument("--val_csv", help="Path to CSV file containing validation data")
