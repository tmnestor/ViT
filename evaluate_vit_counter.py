...
def evaluate_model(model_path, test_csv, test_dir, batch_size=16, output_dir="evaluation_vit", config_path=None):
    ...
    try:
        ...
        try:
            ...
            print("Successfully loaded weights into ViT model!")
        except Exception as e:
            print(f"Could not load directly with load_vit_model: {e}")
            
            # Try creating a new model and loading the state dict directly
            print("Creating new Hugging Face ViT model and loading weights...")
            model = create_vit_transformer(pretrained=False, verbose=False)
            try:
        ...
        config = get_config()
        num_classes = len(config.class_distribution)
        model = create_simple_hf_receipt_counter(num_classes=num_classes, model_type="vit")
        
        # Restore verbosity
        transformers.logging.set_verbosity(prev_verbosity)
        if model is None:
            print("Error: Failed to create even a fallback model")
            return None
