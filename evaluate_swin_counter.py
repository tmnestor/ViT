"""
Swin model evaluation script that uses the unified evaluation module.
"""
...
def main():
    ...
    parser.add_argument("--output_dir", default="evaluation_swin",
    ...
    # Validate that the model file is a Swin model based on filename
    model_filename = str(model_path).lower()
    if "vit" in model_filename:
        print(f"ERROR: You are trying to evaluate a ViT model ({model_path}) with the Swin evaluator.")
        print("Please use evaluate_vit_counter.py for ViT models.")
        sys.exit(1)
    
    # Use the unified evaluation function with "swin" model type
    evaluate_model(
