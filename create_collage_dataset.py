...
def prepare_dataset(
    collage_dir, output_dir, train_split=0.7, val_split=0.15, random_state=42
):
    """
    ...
    train_df, temp_df = train_test_split(
        ...
        if len(df["receipt_count"].unique()) < 10
        else None,
    )
    ...
    val_df, test_df = train_test_split(
        ...
        if len(temp_df["receipt_count"].unique()) < 10
        else None,
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Prepare a dataset from collage images"
    )
    parser.add_argument(
        "--collage_dir",
        default="receipt_collages",
        help="Directory containing collage images",
    )
    parser.add_argument(
        "--output_dir",
        default="receipt_dataset",
    ...
    if total_split >= 1.0:
        raise ValueError(
            f"Train split ({args.train_split}) + validation split ({args.val_split}) must be less than 1.0"  # noqa: E501
        )
    ...
    # print(f"python train_hierarchical_model.py --train_csv {train_csv} --train_dir {os.path.join(args.output_dir, 'train')} --val_csv {val_csv} --val_dir {os.path.join(args.output_dir, 'val')} --model_type vit --output_dir models/hierarchical")  # noqa: E501
